  ...
  <section class="px-4 pb-12">
    ...
    <div class="flex flex-wrap align-middle mt-4">
      <div class="w-full md:w-2/3 my-2 justify-center md:justify-left overflow-x-auto">
        <Paginator :count="meta.count" :visiblePage="5" :pageLength="deposits.length" @pageChanged="refetchDeposit" />
      </div>
      <div class="w-full md:w-1/3 my-2 justify-center md:justify-right pl-2 md:justify-end md:text-right">
        <input @input="filterDeposit($event)" type="text" class="bg-white px-3 py-2 shadow rounded focus:outline-none text-gray-700 text-sm" placeholder="Filter by ID">
      </div>
    </div>
        ...
        <div slot="txid" slot-scope="props">
          <div v-if="props.item.status !== 'cancelled'">
            <div v-if="props.item.benefactor">
              Transfer from <span class="text-green-500">{{ truncate(props.item.benefactor.username, 16) }} </span>
              <button @click="copyToClipboard(props.item.benefactor.username)" class="p-1 focus:outline-none">
                <i class="fas fa-copy"></i>
              </button>
            </div>
            ...
            <button v-else @click="comfirmDesposit(props.item)" class="bg-blue-500 px-2 py-1 text-white rounded disabled:opacity-50">Submit TXID</button>
          </div>
          <div v-else> - </div>
        </div>
      ...
      <div slot="content">
        ...
        <div v-if="step === 1">
          <p class="bg-red-100 text-sm text-red-800 px-4 py-2 rounded">
            Make payment of <b>{{ selectedDeposit.assetAmount }} ETH</b> to the account below!
          </p>
          <div class="flex justify-center items-center">
            <img :src="selectedDeposit.url" alt="address barcode">
          </div>
          <div class="text-center">
            <span class="bg-gray-200 text-sm p-1 rounded text-gray-700">{{ selectedDeposit.address }}</span>  
          </div>
          <div class="mt-10">
  ...
  methods: {
    ...
    async pay () {
      ...
      try {
        ...
        this.$session.set('amount',this.form.amount)
        
        paystackService.transaction.initialize({
          amount: amt,
    ...
    async update () {
      ...
      try {
        ...
        const res = await axios.put(`/updateDeposit/${this.selectedDeposit.id}`,{txid: this.txid})
