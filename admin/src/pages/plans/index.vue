<template>
  <section class="px-4 pb-12">
    <div class="py-6 flex justify-between items-center">
      <div>
        <h1 class="text-xl text-gray-900 mb-2">Plans ({{ meta.count || 0 }})</h1>
        <p class="text-gray-600 leading-tight">Add and manage plans.</p>
      </div>
      <router-link to="/plans/new" class="py-2 px-4 bg-indigo-500 focus:bg-indigo-400 text-white rounded shadow text-sm">
        <span class="fa fa-plus mr-1"></span> New plan
      </router-link>
    </div>
    <div class="flex flex-wrap align-middle mt-4">
      <div class="w-full md:w-2/3 my-2 justify-center md:justify-left overflow-x-auto">
        <Paginator :count="meta.count" :visiblePage="5" :pageLength="plans.length" @pageChanged="fetchPlans" />
      </div>
      <div class="w-full md:w-1/3 my-2 justify-center md:justify-right pl-2 md:justify-end md:text-right">
        <input @input="filterPlans($event)" type="text" class="bg-white px-3 py-2 shadow rounded focus:outline-none text-gray-700 text-sm" placeholder="Filter by ID">
      </div>
    </div>
    <div class="bg-white rounded shadow-md p-1 overflow-x-auto mt-4">
      <TableComponent :columns="table.columns" :items="plans" :options="table.options" :loading="table.loading">
        ...
        <div slot="minimumAmount" slot-scope="props">
          ${{ BigNumber(props.item.minimumAmount).toFormat(2) }}
        </div>
        ...
        <div slot="actions" slot-scope="props">
          <router-link :to="'/plans/' + props.item.id" class="bg-gray-500 px-2 py-1 text-white rounded">
...
export default {
  ...
  methods: {
    filterPlans (e) {
      this.fetchPlans({ search: e.target.value })
    },

    async fetchPlans (options = null) {
      ...
      try {
        const res = await axios.get(`/admin/plans${options ? '?' + options : ''}`)
    ...
    async deletePlan (planId) {
      try {
        const res = await axios.delete(`/admin/plans/${planId}`)

        this.$set(
          this.plans,
          this.plans.findIndex(plan => plan.id === res.data.data.id),
          res.data.data
        )
