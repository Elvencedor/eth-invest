<template>
  <section class="px-4 pb-12">
    <div class="py-6 flex justify-between items-center">
      <div>
        <h1 class="text-xl text-gray-900 mb-2">Add Plan</h1>
        <p class="text-gray-600 leading-tight">Add plan data</p>
      </div>
    </div>
...
export default {
  ...
  methods: {
    async submit () {
      ...
      try {
        // request
        await axios.post(`/admin/plans`, this.plan)

        izitoast.success({
          title: 'Success',
          message: 'New plan added successfull'
        })
      } catch (err) {
        if (!err.response) {
          return izitoast.error({
