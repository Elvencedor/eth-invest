...
export async function checkAuth (req:Request, res:Response, next:Function) {
  if (req.session && req.session.user) {
    userService.fetchUser({ id: req.session.user.id })
      .then(user => {
        if (user) {
      ...
      .catch(err => {
        respond(res, 401, [new AppError({ message: 'Account does not exist!', status: 401 })])
      })
  } else {
    return respond(res, 401, [new AppError({ message: 'Unauthorized!', status: 401 })])
  }
}
